...
from spam import add_to_whitelist, WHITELIST_FILE, load_whitelist, save_whitelist, is_whitelisted
...
def whitelist_number(message):
    ...
    try:
        ...
        if add_to_whitelist(phone):
            bot.send_message(message.chat.id, f"✅ Номер {phone} добавлен в белый список")
        else:
            bot.send_message(message.chat.id, f"❌ Номер {phone} уже в белом списке")
    except IndexError:
        bot.send_message(message.chat.id, "❌ Укажите номер телефона: /whitelist +7XXXXXXXXXX")
...
def handle_message(message):
    try:
        ...
        # Проверяем, находится ли номер в белом списке
        if is_whitelisted(phone_number):
            bot.send_message(message.chat.id, f"❌ Номер {phone_number} находится в белом списке!")
            return
            
        bot.send_message(message.chat.id, f"Спам запущен для {phone_number} на {time} секунд")

        # Запускаем процесс без ожидания завершения
        subprocess.Popen(["python", "spam.py", phone_number, time])

        # Отправляем сообщение о запуске
        bot.send_message(message.chat.id, "✅ Спам успешно запущен!")
